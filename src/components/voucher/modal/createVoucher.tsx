...
const CreateVoucherModal = ({ open, callBack }: CreateVoucherModalProps) => {
    ...
    const [selectedVoucherType, setSelectedVoucherType] = useState<string>("PRODUCT");
    ...
    return (
        <Modal open={showModal} onCancel={() => setShowModal(false)} footer={null} title="Tạo voucher mới">
            ...
            >
                ...
                >
                    <Select defaultValue={"PRODUCT"} onChange={(value: string, options) => setSelectedVoucherType(value)}>
                ...
                >
                    <Select defaultValue={"PERCENT"} onChange={(value: string, options) => setSelectedDiscountType(value)}>
                        <Option value="PERCENT">Phần trăm tổng sản phẩm</Option>
                        <Option value="PER_PRODUCT ">Giá trị cố định trên mỗi sản phẩm</Option>
                ...
                >
                    {refData
                        ?
                        <Button color="blue" className="w-full overflow-hidden" onClick={() => setShowSelectRefModal(true)}>
                            {refData.name}
                        </Button>
                        :
