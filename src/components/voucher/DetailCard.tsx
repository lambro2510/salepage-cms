...
import {
  ...
  Tag,
  Typography,
} from 'antd'; // Import Pagination component
  ...
  const getVoucher = async () => {
    try {
      const response = await http.get(
        `${apiRoutes.voucher}/voucher-store/${storeId}`
      );
    ...
    return (
      <Card key={voucher.voucherCode} style={{ marginBottom: 16 }}>
        <Row gutter={16}>
          ...
          <Col span={4}>
            <Typography.Text>
              {checkStatus(voucher.voucherCodeStatus)}
            </Typography.Text>
          </Col>
          <Col span={5}>
            <Typography.Text>
              {voucher.usedBy == null ? 'Chưa có ai' : voucher.usedBy}
            </Typography.Text>
          </Col>
          ...
          <Col span={5} className="flex justify-center">
            <Typography.Text>
              {convertUTCToVietnamTime(voucher.expireTime)}
            </Typography.Text>
          </Col>
        </Row>
      </Card>
          ...
          <Col span={24}>
            <Card style={{ marginBottom: 16 }}>
              <Row gutter={16}>
                ...
                <Col span={5}>
                  <Typography.Text strong className="flex justify-center">
                    Thời gian sử dụng{' '}
                  </Typography.Text>
                </Col>
                <Col span={5} className="flex justify-center">
