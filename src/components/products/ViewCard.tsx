...
const ViewCard = () => {
  ...
  const showUpload = (product: SellerProductResponse) => {
    modal.confirm({
      ...
      icon: <InfoOutlined />,
      type: 'info',
      content: <UploadImageProduct id={product.id} />,
      okButtonProps: {
        className: 'bg-primary',
      },
      onOk: () => {
  ...
  const columns: ProColumns<SellerProductResponse>[] = [
    ...
    },
    {
      ...
      search: false,
      render: (_: any, row: SellerProductResponse) =>
        row.productCategory?.categoryName,
    },
    {
      title: 'Cửa hàng',
      dataIndex: 'stores',
          ...
          return (
            <div>
              {row.stores.map((store: SellerStoreResponse) => (
                <a
                  type=""
                  onClick={() => navigate(`${webRoutes.stores}/${store.id}`)}
                  key={store.id}
                >
                  {store.storeName}
                </a>
      ...
      <ProTable
        ...
        toolBarRender={() => [
          <Button
            icon={<BiPlus />}
            type="primary"
            onClick={() => navigate(`${webRoutes.products}/create`)}
          >
            Tạo mới
          </Button>,
        ]}
